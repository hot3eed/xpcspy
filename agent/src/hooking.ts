...
export function installHooks(os: string) {
	...
	_installHooks(pointers);
}

function _installHooks(pointers: SymbolicatedPointer[]) {
	for (let pointer of pointers) {
		Interceptor.attach(pointer[1], 
			{ 
				onEnter: function(this: InvocationContext, args: InvocationArguments) {
					_onEnterHandler(pointer[0], args);
				} 
			});
	}
}

const _onEnterHandler = function(symbol: string, args: InvocationArguments): void {
